...
import com.xcion.code.scanner.base.BaseCodeScannerFragment
 ...
 * @author: KernHu
 * @date: 2024/1/23
 * @Description: 扫码基类 fragment
 */
abstract class CodeScannerFragment : BaseCodeScannerFragment<List<Barcode>>() {
    ...
    override fun initUI() {
        val viewfinderViewId: Int = R.id.code_scanner_viewfinder_view
    ...
    override fun createAnalyzer(): Analyzer<List<Barcode>>? {
        var format = arguments?.getInt(ArgsKey.KEY_FORMATS, CodeFormat.FORMATS_ALL.ordinal)
        return when (format) {
